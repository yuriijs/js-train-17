...
function extractTextInParentheses(str) {
  ...
  const array = Array.from(allMatches, (match) => match[1]);
  return array;
...
function countEmails(str) {
  ...
  const allMatches = str.match(regex);
  const amount = allMatches.length;
  return amount;
...
function checkRegexFlags(regex) {
  let flagsReg = regex.flags;
  return flagsReg.includes("g") && flagsReg.includes("m");
...
function replaceWordOccurrences(str, word, newWord) {
  const regex = new RegExp(word, "g");
  const regexAll = str.replaceAll(regex, newWord);
  return regexAll;
...
function findWord(str, word) {
  ...
  let searchWord = str.search(regex);
  return searchWord;
